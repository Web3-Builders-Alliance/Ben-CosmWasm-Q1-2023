...
It provides similar query interface with [Native-Token-Balance-Querier](Native-Token-Balance-Querier) for CW20 token
balance.
...
It also provides the query interface to query available `pool-network` contract info. Any contract can query pair info
to pool network factory contract.
...
It also provides the query interface to query available `pool-network` pair contract info. Any contract can query pair
info to pair contract.
